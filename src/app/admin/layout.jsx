...
const ALL_NAV = [
  {
    label: 'Dashboard', href: '/admin', section: null,
  ...
  {
    label: 'Contacts', href: '/admin/contacts', section: 'contacts',
  ...
  {
    label: 'Portfolio', href: '/admin/portfolio', section: 'portfolio',
  ...
  {
    label: 'Media Library', href: '/admin/media', section: 'media',
    icon: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" /></svg>,
  },
  {
    label: 'Full CMS', href: '/admin/settings', section: 'settings',
  ...
  {
    label: 'Admin Users', href: '/admin/users', section: 'users',
...
function getSession() {
  ...
  try {
    const match = document.cookie.split('; ').find(c => c.startsWith('sw_admin_info='))
    if (!match) return null
    const raw = match.slice('sw_admin_info='.length)
...
export default function AdminLayout({ children }) {
  ...
  useEffect(() => {
    if (session) return
    // Existing sessions without info cookie: fetch from API and it'll set the cookie
    fetch('/api/admin/me')
  ...
  const SKIP_LAYOUT = ['/admin/login', '/admin/forgot-password', '/admin/reset-password']
  if (SKIP_LAYOUT.some(p => pathname.startsWith(p))) return <>{children}</>
  ...
  const visibleNav = ALL_NAV.filter(item => {
    ...
    return session?.permissions?.[item.section] === true
  })
  ...
  const currentLabel = ALL_NAV.find(n => n.href === pathname || (n.href !== '/admin' && pathname.startsWith(n.href)))?.label || 'Admin'

  return (
    <div className="min-h-screen bg-gray-50 flex">
      {sidebarOpen && <div className="fixed inset-0 bg-black/50 z-20 lg:hidden" onClick={() => setSidebarOpen(false)} />}

      <aside className={`fixed top-0 left-0 h-full w-64 bg-slate-900 z-30 flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        <div className="px-5 py-5 border-b border-white/10">
          ...
          {session && (
            <div className="mt-3 flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-gold-500/20 border border-gold-500/30 flex items-center justify-center text-gold-300 text-xs font-bold">
                {session.name?.[0]?.toUpperCase() || 'A'}
              </div>
              <div className="min-w-0">
                <p className="text-white text-xs font-semibold truncate">{session.name}</p>
                <p className="text-slate-500 text-[9px]">{isSuperAdmin ? 'Super Admin' : 'Admin'}</p>
              </div>
            </div>
          )}
        </div>
          ...
          {visibleNav.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${active ? 'bg-gold-500/15 text-gold-300 border border-gold-500/20' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}>
        ...
        <div className="px-3 py-4 border-t border-white/10 space-y-0.5">
          <Link href="/admin/change-password" onClick={() => setSidebarOpen(false)}
          ...
          <Link href="/" target="_blank" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-slate-400 hover:text-white hover:bg-white/5 transition-all">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" /></svg>
            View Website
          </Link>
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-slate-400 hover:text-gold-300 hover:bg-gold-500/10 transition-all">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" /></svg>
            Logout
          </button>
        </div>
      </aside>
