...
import { PHONE_NUMBER, WHATSAPP_NUMBER, EMAIL, FACEBOOK_URL } from '@/shared/constants/constants'
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
          ...
          <div className="lg:col-span-2">
            <Link href="/" className="inline-flex items-center gap-2.5 mb-5">
              <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-red-600 to-orange-500 flex items-center justify-center shadow-md shadow-red-600/30">
                <span className="text-white font-black text-base">SW</span>
              </div>
              <div>
                <span className="font-bold text-white text-sm leading-none block">Signature Works BD</span>
                <span className="text-[10px] text-slate-400 leading-none mt-0.5 block">Design & Construction</span>
              </div>
            </Link>
            ...
            <ul className="space-y-3">
              ...
              <li className="flex items-center gap-3">
                ...
                <a href={`tel:${PHONE_NUMBER}`} className="text-slate-400 text-sm hover:text-red-400 transition-colors">{PHONE_NUMBER}</a>
              </li>
              <li className="flex items-center gap-3">
                <svg className="w-4 h-4 text-red-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
                <a href={`mailto:${EMAIL}`} className="text-slate-400 text-sm hover:text-red-400 transition-colors">{EMAIL}</a>
              </li>
              <li className="flex items-center gap-3">
          ...
          {columns.map((col) => (
            <div key={col.heading}>
              ...
              <ul className="space-y-3">
                {col.links.map((link) => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-slate-400 text-sm hover:text-red-400 transition-colors duration-200">
                      {link.label}
                    </Link>
                  </li>
                ))}
        ...
        <Container className="py-5">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-slate-400 text-sm">
              ...
              <span className="font-semibold bg-gradient-to-r from-red-500 to-orange-400 bg-clip-text text-transparent">
                Signature Works BD
              </span>
              {'. All rights reserved.'}
            </p>
