      ...
      <Container>
          ...
          {pricingData.map((pkg, i) => (
            <Reveal key={pkg.id} delay={i * 100}>
              <div className={`relative rounded-2xl p-8 flex flex-col transition-all duration-300 ${
                pkg.popular
                  ? 'bg-slate-900 dark:bg-slate-900 border-2 border-gold-500/40 shadow-2xl shadow-gold-500/10 md:scale-105'
                  : 'bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-white/[0.07] hover:border-gold-200 dark:hover:border-gold-500/20 hover:shadow-lg hover:shadow-gold-500/5'
              }`}>
                {pkg.popular && (
                  <>
                    {/* Top red glow line */}
                    <div className="absolute top-0 inset-x-0 h-1 rounded-t-2xl bg-gradient-to-r from-navy-900 via-gold-500 to-amber-300" />
                    {/* Badge */}
                    <div className="absolute -top-4 left-1/2 -translate-x-1/2 px-5 py-1.5 rounded-full bg-gradient-to-r from-navy-900 to-gold-500 text-white text-xs font-bold tracking-widest uppercase shadow-lg shadow-gold-500/25">
                      Most Popular
                    </div>
                  </>
                )}
                    ...
                    <li key={f} className="flex items-center gap-2.5 text-sm">
                      <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${
                        pkg.popular
                          ...
                          : 'bg-gold-50 dark:bg-gold-500/10 text-gold-600 dark:text-gold-300'
                      }`}>
        ...
        <Reveal>
          <p className="text-center text-slate-400 dark:text-slate-500 text-sm mt-10">
            ...
            <a href="/#contact" className="inline-flex items-center gap-1.5 text-gold-600 hover:text-gold-500 font-semibold hover:underline transition-colors">
